...
# # Custom class
# class Offload:
#     def __init__(self, raspberry, factor, function):
#         self.raspberry = raspberry
#         self.factor = factor
#         self.function = function
#
#     def __repr__(self):
#         return repr((self.raspberry, self.factor))


# # a list of timing objects
# L = [Offload('Rasp1', rasp1_factor()),
#      Offload('Rasp2', rasp2_factor()),
#      Offload('Rasp3', rasp3_factor())]
#
# M = [Offload('ac_control', ac_control_factor()),
#      Offload('speed', speed_factor())]
#
# N = [Offload('accuracy', accuracy_factor()),
#      Offload('classification', classification_factor()),
#      Offload('confusion', confusion_factor())]
#
# O = [Offload('processing', processing_factor())]
#
# total_min = min(L, key=lambda raspberry: raspberry.factor)
# total_max = max(L, key=lambda raspberry: raspberry.factor)
# rasp1_min = min(M, key=lambda raspberry: raspberry.factor)
# rasp1_max = max(M, key=lambda raspberry: raspberry.factor)
# rasp2_min = min(N, key=lambda raspberry: raspberry.factor)
# rasp2_max = max(N, key=lambda raspberry: raspberry.factor)
# rasp3_min = min(O, key=lambda raspberry: raspberry.factor)
# rasp3_max = max(O, key=lambda raspberry: raspberry.factor)
